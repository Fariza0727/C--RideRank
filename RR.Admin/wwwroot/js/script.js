...
(function () {
    ...
    $(function () {
        ...
        if (plugins.bootstrapCollapse.length) {
            for (i = 0; i < plugins.bootstrapCollapse.length; i++) {
        ...
        for (z = 0; z < plugins.lightGalleryItem.length; z++) {
        ...
        if (plugins.lightGallery.length) {
            for (i = 0; i < plugins.lightGallery.length; i++) {
                initLightGallery(plugins.lightGallery[i]);
            }
        }

        if (plugins.lightGalleryItem.length) {
            for (i = 0; i < plugins.lightGalleryItem.length; i++) {
                initLightGalleryItem(plugins.lightGalleryItem[i]);
            }
        }

        if (plugins.lightDynamicGalleryItem.length) {
            for (i = 0; i < plugins.lightDynamicGalleryItem.length; i++) {
        ...
        if (plugins.rdNavbar.length) {
            var aliaces, j, len, value, values, responsiveNavbar;
        ...
        if (plugins.inlineToggle.length) {
            for (i = 0; i < plugins.inlineToggle.length; i++) {
        ...
        if (plugins.focusToggle.length) {
            for (i = 0; i < plugins.focusToggle.length; i++) {
                $element = $(plugins.focusToggle[i]);
        ...
        if (plugins.countDown.length) {
            for (i = 0; i < plugins.countDown.length; i++) {
                ...
                var format = $countDownItem.attr('data-format');
                var settings = [];
        ...
        if (plugins.doughnutChart.length) {
            for (i = 0; i < plugins.doughnutChart.length; i++) {
                $element = $(plugins.doughnutChart[i]),
                ...
                if ($chartDataList) {
                    ...
                    for (k = 0; k < chartDataListItems.length; k++) {
                        value = Math.abs(chartDataListItems[k].getAttribute('data-value') * 1);
                        ratioValues.push({});
        ...
        if (plugins.radioPanel) {
            for (i = 0; i < plugins.radioPanel.length; i++) {
                $element = $(plugins.radioPanel[i]);
